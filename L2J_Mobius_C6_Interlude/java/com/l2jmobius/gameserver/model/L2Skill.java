...
import com.l2jmobius.gameserver.skills.l2skills.L2SkillChargeEffect;
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst, L2Character target)
	{
		...
		switch (_targetType)
		{
			...
			case TARGET_AREA:
			{
				...
				final boolean effectOriginIsL2PlayableInstance = cha instanceof L2Playable;
				L2PcInstance src = null;
				if (activeChar instanceof L2PcInstance)
				{
					src = (L2PcInstance) activeChar;
				...
				for (L2Object obj : activeChar.getKnownList().getKnownObjects().values())
				{
					...
					if (!target.isAlikeDead() && (target != activeChar))
					{
						...
						else if (effectOriginIsL2PlayableInstance && // If effect starts at L2PlayableInstance and
			...
			case TARGET_ENEMY_SUMMON:
			{
				if ((target != null) && (target instanceof L2Summon))
				{
					...
					if (((activeChar instanceof L2PcInstance) && (activeChar.getPet() != targetSummon) && !targetSummon.isDead() && ((targetSummon.getOwner().getPvpFlag() != 0) || (targetSummon.getOwner().getKarma() > 0) || targetSummon.getOwner().isInDuel())) || (targetSummon.getOwner().isInsideZone(ZoneId.PVP) && ((L2PcInstance) activeChar).isInsideZone(ZoneId.PVP)))
					{
						return new L2Character[]
						{
							targetSummon
