...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private void thinkActive()
	{
		...
		else if (!(npc instanceof L2ChestInstance) && (npc.getSpawn() != null) && (Rnd.nextInt(RANDOM_WALK_RATE) == 0))
		{
			...
			{
				if ((Config.MONSTER_RETURN_DELAY > 0) && (npc instanceof L2MonsterInstance) && !npc.isAlikeDead() && !npc.isDead() && (npc.getSpawn() != null) && !npc.isInsideRadius(npc.getSpawn().getX(), npc.getSpawn().getY(), Config.MAX_DRIFT_RANGE, false))
				{
	...
	private void thinkAttack()
	{
		 ...
		 * "L2AttackableAI: thinkAttack() faction call failed: " + e.getMessage()); } }
		 */
