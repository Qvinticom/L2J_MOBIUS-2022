...
import com.l2jmobius.gameserver.ai.L2PlayerAI.IntentionCommand;
...
public class L2CharacterAI extends AbstractAI
{
	...
	protected void onIntentionCast(L2Skill skill, L2Object target)
	{
		...
		if ((target instanceof L2PcInstance) && (_actor instanceof L2PcInstance))
		{
			if ((((L2PcInstance) _actor).getKarma() > 0) && ((_actor.getLevel() - ((L2PcInstance) target).getLevel()) >= 10) && ((L2Playable) target).getProtectionBlessing() && !((L2Character) target).isInsideZone(ZoneId.PVP))
			{
				// If attacker have karma and have level >= 10 than his target and target have Newbie Protection Buff,
				clientActionFailed();
				return;
	...
	protected void onIntentionMoveToInABoat(Location destination, Location origin)
	{
		...
		// Set the Intention of this AbstractAI to AI_INTENTION_MOVE_TO
		//
		// changeIntention(AI_INTENTION_MOVE_TO, new Location(((L2PcInstance)_actor).getBoat().getX() - destination.x, ((L2PcInstance)_actor).getBoat().getY() - destination.y, ((L2PcInstance)_actor).getBoat().getZ() - destination.z, 0) , null);
