	...
	public void reduceHp(double amount, Creature attacker, boolean awake)
	{
		...
		if (attacker instanceof Player)
		{
			if (getActiveChar().isInDuel())
			{
				...
				if (((Player) attacker).getDuelId() != getActiveChar().getDuelId())
				{
		...
		{
			// if attacked by a non Player & non Summon the duel gets canceled
			if (getActiveChar().isInDuel() && !(attacker instanceof Servitor))
			{
				getActiveChar().setDuelState(Duel.DUELSTATE_INTERRUPTED);
			}
		...
		if ((attacker != null) && (attacker != getActiveChar()) && (fullValue > 0))
		{
			...
			{
				smsg.addNpcName(((Npc) attacker).getTemplate().getDisplayId());
			}
