...
public class RequestEnchantItem extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		{
			activeChar.sendPacket(new SystemMessage(1688));
		...
		if ((item.getItem().getItemType() == L2WeaponType.ROD) || (!Config.ALLOW_HERO_ENCHANT && (item.getItemId() >= 6611) && (item.getItemId() <= 6621)) || ((item.getItemId() >= 7816) && (item.getItemId() <= 7831)))
		{
		...
		{
			for (final int crystalscroll : crystalScrolls)
			{
