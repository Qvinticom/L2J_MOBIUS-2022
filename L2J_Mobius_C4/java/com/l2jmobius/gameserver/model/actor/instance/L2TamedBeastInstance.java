...
public final class L2TamedBeastInstance extends L2FeedableBeastInstance
{
	...
	public void setOwner(L2PcInstance owner)
	{
		if (owner != null)
		{
			...
			{
				// if the skill is a buff, check if the owner has it already [ owner.getFirstEffect(L2Skill skill) ]
	...
	public void onOwnerGotAttacked(L2Character attacker)
	{
		...
		if (HPRatio >= 0.8)
		{
			...
			{
				// if the skill is a debuff, check if the attacker has it already [ attacker.getFirstEffect(L2Skill skill) ]
		...
		else if (HPRatio < 0.5)
		{
			...
			{
				// if the skill is a buff, check if the owner has it already [ owner.getFirstEffect(L2Skill skill) ]
				if ((Rnd.get(5) < chance) && ((skill.getSkillType() == L2Skill.SkillType.HEAL) || (skill.getSkillType() == L2Skill.SkillType.HOT) || (skill.getSkillType() == L2Skill.SkillType.BALANCE_LIFE) || (skill.getSkillType() == L2Skill.SkillType.HEAL_PERCENT) || (skill.getSkillType() == L2Skill.SkillType.HEAL_STATIC) || (skill.getSkillType() == L2Skill.SkillType.COMBATPOINTHEAL) || (skill.getSkillType() == L2Skill.SkillType.CPHOT) || (skill.getSkillType() == L2Skill.SkillType.MANAHEAL) || (skill.getSkillType() == L2Skill.SkillType.MANA_BY_LEVEL) || (skill.getSkillType() == L2Skill.SkillType.MANARECHARGE) || (skill.getSkillType() == L2Skill.SkillType.MPHOT)))
				{
	...
	private class CheckDuration implements Runnable
	{
		...
		public void run()
		{
			...
			if ((item != null) && (item.getCount() >= 1))
			{
				...
				owner.callSkill(SkillTable.getInstance().getInfo(foodTypeSkillId, 1), targets, false);
	...
	private class CheckOwnerBuffs implements Runnable
	{
		...
		public void run()
		{
			...
			final FastMap<Integer, L2Skill> skills = (FastMap<Integer, L2Skill>) _tamedBeast.getTemplate().getSkills();
