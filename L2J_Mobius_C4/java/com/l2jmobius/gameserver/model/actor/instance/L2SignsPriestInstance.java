...
public class L2SignsPriestInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			int stoneType = 0;
			final L2ItemInstance ancientAdena = player.getInventory().getItemByItemId(SevenSigns.ANCIENT_ADENA_ID);
			...
			switch (val)
			{
				case 2: // Purchase Record of the Seven Signs
					...
					final L2ItemInstance adenaItem = player.getInventory().getAdenaInstance();
					...
					final L2ItemInstance recordSevenSigns = player.getInventory().addItem("SevenSigns", SevenSigns.RECORD_SEVEN_SIGNS_ID, 1, player, this);
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
					...
					if (cabal == SevenSigns.CABAL_DAWN)
					{
						player.sendPacket(new SystemMessage(SystemMessage.SEVENSIGNS_PARTECIPATION_DAWN)); // Joined Dawn
					}
					else
					{
						player.sendPacket(new SystemMessage(SystemMessage.SEVENSIGNS_PARTECIPATION_DUSK)); // Joined Dusk
					}
					...
					switch (newSeal)
					{
						case SevenSigns.SEAL_AVARICE:
							player.sendPacket(new SystemMessage(SystemMessage.FIGHT_FOR_AVARICE));
							break;
						case SevenSigns.SEAL_GNOSIS:
							player.sendPacket(new SystemMessage(SystemMessage.FIGHT_FOR_GNOSIS));
							break;
						case SevenSigns.SEAL_STRIFE:
							player.sendPacket(new SystemMessage(SystemMessage.FIGHT_FOR_STRIFE));
							break;
					}
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
					...
					{
						player.sendPacket(new SystemMessage(SystemMessage.CONTRIB_SCORE_EXCEEDED));
						break;
					}
					...
					sm = new SystemMessage(SystemMessage.CONTRIB_SCORE_INCREASED);
				...
				case 20: // Seal Status (for when joining a cabal)
					final TextBuilder contentBuffer = new TextBuilder("<html><body><font color=\"LEVEL\">[ Seal Status ]</font><br>");
