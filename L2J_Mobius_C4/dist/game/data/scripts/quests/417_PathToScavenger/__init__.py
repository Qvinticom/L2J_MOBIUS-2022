...
class Quest (JQuest) :
 ...
 def onTalk (Self,npc,st):
   ...
   if id == CREATED :
     ...
     st.set("onlyone","0")
     st.set("id","0")
   if npcId == 7524 and st.getInt("cond")==0 :
     htmltext = "7524-01.htm"
   elif npcId == 7524 and st.getInt("cond") and st.getQuestItemsCount(PIPIS_LETTER_ID)>0 :
          htmltext = "7524-06.htm"
   elif npcId == 7524 and st.getInt("cond") and st.getQuestItemsCount(PIPIS_LETTER_ID)==0 and id==STARTING :
          htmltext = "7524-01.htm"
   elif npcId == 7524 and st.getInt("cond") and st.getQuestItemsCount(PIPIS_LETTER_ID)==0 :
          htmltext = "7524-07.htm"
   elif npcId == 7519 and st.getInt("cond") and st.getQuestItemsCount(PIPIS_LETTER_ID)>0 :
          htmltext = "7519-01.htm"
   elif npcId == 7519 and st.getInt("cond") and ((st.getQuestItemsCount(CHALIS_AXE_ID)+st.getQuestItemsCount(BRONKS_INGOT_ID)+st.getQuestItemsCount(ZIMENFS_POTION_ID))==1) and ((st.getInt("id") / 10)==0) :
          htmltext = "7519-05.htm"
   elif npcId == 7519 and st.getInt("cond") and ((st.getQuestItemsCount(CHALIS_AXE_ID)+st.getQuestItemsCount(BRONKS_INGOT_ID)+st.getQuestItemsCount(ZIMENFS_POTION_ID))==1) and ((st.getInt("id") / 10)>0) :
          htmltext = "7519-08.htm"
   elif npcId == 7519 and st.getInt("cond") and ((st.getQuestItemsCount(CHALIS_PAY_ID)+st.getQuestItemsCount(BRONKS_PAY_ID)+st.getQuestItemsCount(ZIMENFS_PAY_ID))==1) and (st.getInt("id") < 50) :
          ...
          st.takeItems(BRONKS_PAY_ID,1)
          st.set("cond","4")
   elif npcId == 7519 and st.getInt("cond") and st.getQuestItemsCount(MIONS_LETTER_ID) :
          htmltext = "7519-13.htm"
   elif npcId == 7519 and st.getInt("cond") and (st.getQuestItemsCount(BEAR_PIC_ID) or st.getQuestItemsCount(TARANTULA_PIC_ID) or st.getQuestItemsCount(BEAD_PARCEL_ID) or st.getQuestItemsCount(ROUTS_TP_SCROLL_ID) or st.getQuestItemsCount(SUCCUBUS_UNDIES_ID)) :
          htmltext = "7519-14.htm"
   elif npcId == 7517 and st.getInt("cond") and st.getQuestItemsCount(CHALIS_AXE_ID)==1 and st.getInt("id") < 20 :
          ...
          if st.getInt("id") >= 50 :
            st.set("cond","3")
          st.set("id",str(st.getInt("id")+10))
   elif npcId == 7517 and st.getInt("cond") and st.getQuestItemsCount(CHALIS_PAY_ID)==1 :
          htmltext = "7517-03.htm"
   elif npcId == 7525 and st.getInt("cond") and st.getQuestItemsCount(BRONKS_INGOT_ID)==1 and st.getInt("id") < 20 :
          ...
          if st.getInt("id") >= 50 :
            st.set("cond","3")          
          st.set("id",str(st.getInt("id")+10))
   elif npcId == 7525 and st.getInt("cond") and st.getQuestItemsCount(BRONKS_PAY_ID)==1 :
          htmltext = "7525-03.htm"
   elif npcId == 7538 and st.getInt("cond") and st.getQuestItemsCount(ZIMENFS_POTION_ID)==1 and st.getInt("id") < 20 :
          ...
          if st.getInt("id") >= 50 :
            st.set("cond","3")
          st.set("id",str(st.getInt("id")+10))
   elif npcId == 7538 and st.getInt("cond") and st.getQuestItemsCount(ZIMENFS_PAY_ID)==1 :
          htmltext = "7538-03.htm"
   elif npcId == 7556 and st.getInt("cond") and st.getQuestItemsCount(MIONS_LETTER_ID)==1 :
          htmltext = "7556-01.htm"
          st.takeItems(MIONS_LETTER_ID,1)
          st.giveItems(BEAR_PIC_ID,1)
          st.set("cond","5")
          st.set("id",str(0))
   elif npcId == 7556 and st.getInt("cond") and st.getQuestItemsCount(BEAR_PIC_ID)==1 and st.getQuestItemsCount(HONEY_JAR_ID)<5 :
          ...
          st.set("cond","7")
   elif npcId == 7556 and st.getInt("cond") and st.getQuestItemsCount(TARANTULA_PIC_ID)==1 and st.getQuestItemsCount(BEAD_ID)<20 :
          ...
          st.set("cond","9")
   elif npcId == 7556 and st.getInt("cond") and st.getQuestItemsCount(BEAD_PARCEL_ID)>0 :
          htmltext = "7556-06.htm"
   elif npcId == 7556 and st.getInt("cond") and (st.getQuestItemsCount(ROUTS_TP_SCROLL_ID)>0 or st.getQuestItemsCount(SUCCUBUS_UNDIES_ID)>0) :
          htmltext = "7556-07.htm"
   elif npcId == 7316 and st.getInt("cond") and st.getQuestItemsCount(BEAD_PARCEL_ID)==1 :
          htmltext = "7316-01.htm"
   elif npcId == 7316 and st.getInt("cond") and st.getQuestItemsCount(ROUTS_TP_SCROLL_ID)==1 :
          htmltext = "7316-04.htm"
   elif npcId == 7316 and st.getInt("cond") and st.getQuestItemsCount(SUCCUBUS_UNDIES_ID)==1 :
   ...
   elif npcId == 7557 and st.getInt("cond") and st.getQuestItemsCount(ROUTS_TP_SCROLL_ID)==1 :
          htmltext = "7557-01.htm"
   return htmltext

 def onKill (self,npc,player,isPet):
   st = player.getQuestState("417_PathToScavenger")
   if st :
      if st.getState() != STARTED : return
      npcId = npc.getNpcId()
      if npcId == 777 :
        if st.getInt("cond") and st.getQuestItemsCount(BEAR_PIC_ID) == 1 and st.getQuestItemsCount(HONEY_JAR_ID) < 5 :
          if st.getInt("id") > 20 :
        ...
        if st.getInt("cond") and st.getQuestItemsCount(TARANTULA_PIC_ID) == 1 and st.getQuestItemsCount(BEAD_ID) < 20 :
          if npc.isSpoil() :
            if st.getRandom(2) == 0 :
...
QUEST.addTalkId(7517)
QUEST.addTalkId(7519)
QUEST.addTalkId(7524)
QUEST.addTalkId(7525)
QUEST.addTalkId(7538)
QUEST.addTalkId(7556)
QUEST.addTalkId(7557)

QUEST.addKillId(403)
QUEST.addKillId(5058)
QUEST.addKillId(508)
QUEST.addKillId(777)