...
#Mobs
SCAVENGER_WERERAT = 39
TUREK_WARHOUND = 494
TYRANT_KINGPIN = 193
TRISALIM_TARANTULA = 561
...
class Quest (JQuest) :
 ...
 def __init__(self,id,name,descr):
     ...
     self.questItemIds = [SCAVENGER_WERERAT_SKULL, TUREK_WARHOUND_TAIL, TYRANT_KINGPIN_HEART, TRISALIM_TARANTULAS_VENOM_SAC, MANUAL_OF_MANACLES, PENITENTS_MANACLES, PENITENTS_MANACLES2, PENITENTS_MANACLES1]
 ...
 def onTalk (self,npc,st):
   ...
   npcId = npc.getNpcId()
   id = st.getState()
   if npcId == 7981 : #Black Judge
       if id == CREATED :
           if st.getPlayer().getPkKills() >= 1:
               htmltext = "7981-02.htm"
           else:
           ...
           if st.getQuestItemsCount(PENITENTS_MANACLES) > 0 :
               plevel = findPetLvl(st.getPlayer(),PENITENTS_MANACLES)
               level = st.getPlayer().getLevel()
               if st.getInt("level") > level :
                   level = st.getInt("level")
               if plevel :
                   if plevel > level:
                       htmltext = "7981-13.htm"
                   else :
                       htmltext = "7981-12.htm"
               else :
                   htmltext = "7981-12.htm"
           else :
               htmltext = "7981-18.htm"
   elif npcId == 7668 : # Katari
       if condition == 2 :
           st.set("cond","6")
...
QUEST.addStartNpc(7981)

QUEST.addTalkId(7981)
QUEST.addTalkId(7668)
QUEST.addTalkId(7597)
QUEST.addTalkId(7612)
QUEST.addTalkId(7718)
QUEST.addTalkId(7300)
