...
DROPLIST={AMBER_BASILISK:[AMBER_SCALE,15],WHISPERING_WIND:[WIND_SOULSTONE,20],GLASS_JAGUAR:[GLASS_EYE,35],HORROR_MIST_RIPPER:[HORROR_ECTOPLASM,15],
          SILENOS:[SILENOS_HORN,30],ANT_SOLDIER:[ANT_SOLDIER_APHID,40],ANT_WARRIOR_CAPTAIN:[ANT_SOLDIER_APHID,40],TYRANT:[TYRANTS_CHITIN,50],
 ...
 def onAdvEvent (self,event,npc,player):
   ...
   if event == "7738-06.htm":
     if st.getQuestItemsCount(WISH_POTION) :
       ...
       if st.getQuestItemsCount(MATILDS_ORB) : htmltext = "7738-12.htm"
 ...
 def onKill(self,npc,player,isPet):
   ...
   if npcId == SECRET_KEEPER_TREE and cond == 1 and not st.getQuestItemsCount(SECRET_BOOK):
     ...
     if npcId == SUCCUBUS_OF_SEDUCTION:
       ...
       try :
         if st.getQuestTimer("sanches_timer1") != None :
       ...
       try :
         if st.getQuestTimer("bonaparterius_timer1") != None :
       ...
       try :
         if st.getQuestTimer("ramsebalius_timer1") != None :
       ...
       try :
         if st.getQuestTimer("greatdemon_timer1") != None :
