...
class Quest (JQuest) :
 ...
 def onTalk (Self,npc,st):
   ...
   if id == CREATED :
     ...
     st.set("onlyone","0")
     st.set("id","0")
   if npcId == 7379 and st.getInt("cond")==0 :
     htmltext = "7379-01.htm"
   elif npcId == 7379 and st.getInt("cond") :
   ...
   elif npcId == 7425 and st.getInt("cond") and st.getQuestItemsCount(BEZIQUES_LETTER_ID)>0 :
        htmltext = "7425-01.htm"
   elif npcId == 7425 and st.getInt("cond") and st.getQuestItemsCount(HORSESHOE_OF_LIGHT_ID)==0 and st.getQuestItemsCount(BEZIQUES_LETTER_ID)==0 :
        if st.getQuestItemsCount(SPATOIS_BONES_ID)<10 :
          ...
          st.set("cond","4")
   elif npcId == 7425 and st.getInt("cond") and st.getQuestItemsCount(HORSESHOE_OF_LIGHT_ID)>0 :
        htmltext = "7425-08.htm"
   return htmltext

 def onKill (self,npc,player,isPet):
   st = player.getQuestState("403_PathToRogue")
   if st :
     ...
     if st.getItemEquipped(7) == NETIS_BOW_ID or st.getItemEquipped(7) == NETIS_DAGGER_ID :
       if npcId in (35, 42, 45, 51, 54, 60) :
          st.set("id","0")
          if st.getInt("cond") and st.getQuestItemsCount(SPATOIS_BONES_ID)<10 and st.getRandom(10)<DROP_CHANCE[npcId] :
          ...
          st.set("id","0")
          if st.getInt("cond") and st.getQuestItemsCount(WANTED_BILL_ID)>0 :
              n = st.getRandom(4)
...
QUEST.addTalkId(7425)

QUEST.addKillId(5038)
