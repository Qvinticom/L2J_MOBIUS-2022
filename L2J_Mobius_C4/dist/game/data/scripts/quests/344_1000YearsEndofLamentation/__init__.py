...
class Quest (JQuest) :
 ...
 def onEvent (self,event,st) :
     ...
     cond = st.getInt("cond")
     if event == "7754-04.htm" :
        if st.getPlayer().getLevel()>=48 and cond == 0 :
          st.setState(STARTED)
          st.set("cond","1")
          ...
          htmltext = default
          st.exitQuest(1)
     elif event == "7754-08.htm" :     
         st.exitQuest(1)
         st.playSound("ItemSound.quest_finish")
     elif event == "7754-06.htm" and cond == 1 :
       if not amount :
     ...
     elif event == "7754-11.htm" and cond == 1 :
         if st.getInt("ok") != 1:
            htmltext=default
         else :
 ...
 def onTalk (self,npc,st):
     ...
     if id == CREATED : 
        ...
        if amount :
           ...
           htmltext = "7754-09.htm"
     elif cond==2 :
        if npcId == GILMORE :
          htmltext="7754-15.htm"
 ...
 def onKill(self,npc,player,isPet):
     ...
     if st.getState() != STARTED :
         return
     npcId = npc.getNpcId()
     chance = (CHANCE+(npcId-234)*2)*Config.RATE_DROP_QUEST
