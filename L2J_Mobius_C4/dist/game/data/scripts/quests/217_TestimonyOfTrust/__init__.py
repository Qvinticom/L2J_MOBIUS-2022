...
class Quest (JQuest) :
 ...
 def onKill (self,npc,player,isPet):
   st = player.getQuestState("217_TestimonyOfTrust")
   if st :
     ...
     if cond == 2 and npcId in [ 13, 19, 36, 44 ] :          # Condition 2 kill the Luel of Zephy and Aktea of the Woods
       if npcId in [ 36,44 ] and st.getQuestItemsCount(BREATH_OF_WINDS_ID) == 0 :
         ...
         if st.getRandom(100)<(st.getInt("id")*33) :
           ...
           st.addSpawn(5120,9410,50301,-3713,600000)   ### FIXME ### Temp fix for spawn
         ...
         if st.getRandom(100)<(st.getInt("id")*33) :
           st.playSound("Itemsound.quest_before_battle")
           st.addSpawn(5121,16895,47210,-3673,600000)  ### FIXME ### Temp fix for spawn
...
QUEST.addTalkId(7191)
QUEST.addTalkId(7358)
QUEST.addTalkId(7464)
QUEST.addTalkId(7515)
QUEST.addTalkId(7531)
QUEST.addTalkId(7565)
QUEST.addTalkId(7621)
QUEST.addTalkId(7657)
