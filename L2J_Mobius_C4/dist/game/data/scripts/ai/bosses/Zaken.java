...
public class Zaken extends Quest
{
	...
	public Zaken(int questId, String name, String descr)
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new Runnable()
		{
			...
			public void run()
			{
				if (GetTimeHour() == 0)
				{
					final L2DoorInstance door = DoorTable.getInstance().getDoor(21240006);
					if (door != null)
					{
						door.openMe();
					}
		...
		if (status == DEAD)
		{
			...
			else
			{
				...
				final L2GrandBossInstance zaken = (L2GrandBossInstance) addSpawn(ZAKEN, 55275, 218880, -3217, 0, false, 0);
	...
	public String onAttack(L2NpcInstance npc, L2PcInstance attacker, int damage, boolean isPet)
	{
		...
		if (npcId == ZAKEN)
		{
			...
			if ((GetTimeHour() > 4) && (npc.getCurrentHp() < ((npc.getMaxHp() * _quest2) / 4)))
			{
				_quest2 = (_quest2 - 1);
				final int i2 = Rnd.get(15);
				_ai1 = Xcoords[i2] + Rnd.get(650);
