...
public abstract class AbstractSagaQuest extends Quest
{
	...
	private void giveHalishaMark(QuestState st2)
	{
		...
		{
			if (getQuestItemsCount(st2.getPlayer(), Items[3]) >= 700)
			{
				takeItems(st2.getPlayer(), Items[3], 20);
				int xx = st2.getPlayer().getX();
				int yy = st2.getPlayer().getY();
				int zz = st2.getPlayer().getZ();
				// TODO: Check if spawns correct
				L2Npc Archon = addSpawn(Mob[1], xx, yy, zz, 0 + 32500, true, -1);
				addSpawn(st2, Archon);
				st2.set("spawned", "1");
			...
			{
				giveItems(st2.getPlayer(), Items[3], getRandom(1, 4));
			}
		}
	}
		...
		if (st != null)
		{
			switch (event)
			{
				...
				case "Mob_3 Timer 1":
					...
					if ((Mob_2 != null) && (npc.calculateDistance(Mob_2, false, false) < 450))
					{
		...
		for (int Guardian_Angel = 27214; Guardian_Angel < 27217; Guardian_Angel++)
		{
			if (npcId == Guardian_Angel)
			{
				...
				if ((st1 != null) && st1.isCond(6))
				{
					...
					{
						giveItems(st1.getPlayer(), Items[5], 1);
						st.setCond(7, true);
					}
				}
				return super.onKill(npc, player, isSummon);
	...
	{
		npc.broadcastPacket(new NpcSay(npc.getObjectId(), ChatType.NPC_GENERAL, npc.getId(), text));
	}
