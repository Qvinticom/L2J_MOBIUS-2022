...
{
	private static Logger _log = Logger.getLogger(Player.class.getName());
	...
	private Player _currentTransactionRequester;
		...
		try
		{
			switch (getCurrentState())
			{
				...
				case INTERACT:
				{
					...
					{
						final Player temp = (Player) getTarget();
						sendPacket(new ActionFailed());
	...
	private void doPickupItem()
	{
		...
		if (!isInParty())
		{
			...
			final Item target2 = getInventory().addItem(target);
	...
	{
		final Item weapon = getInventory().getPaperdollItem(7);
		if (weapon == null)
		{
	...
	public void reduceCurrentHp(int amount, Creature attacker, boolean sendMessage)
	{
		...
		if (sendMessage)
		{
			...
			if ((attacker instanceof Monster) || (attacker instanceof Npc))
			{
				final int mobId = ((Npc) attacker).getNpcTemplate().getNpcId();
	...
	public void setTransactionRequester(Player requestor)
	{
		_currentTransactionRequester = requestor;
	}
	
	public Player getTransactionRequester()
	{
	...
	public void addKnownObject(WorldObject object)
	{
		...
		{
			sendPacket(new SpawnItem((Item) object));
		}
		else if (object instanceof Npc)
		{
			ni = new NpcInfo((Npc) object);
			sendPacket(ni);
		}
		else if (object instanceof Pet)
		{
			ni = new NpcInfo((Pet) object);
			sendPacket(ni);
		}
	...
	public void deleteMe()
	{
		...
		if (getClanId() != 0)
		{
			getClan().getClanMember(getName()).setPlayer(null);
		}
	...
	{
		final Item arrows = getInventory().destroyItem(getInventory().getPaperdollObjectId(8), 1);
